...
const KOBO_FORM_1A5b_FORM_ID = process.env.KOBO_FORM_1A5b_UID;
//const KOBO_FORM_1A5b_FORM_ID = process.env.KOBO_FORM_1A5b_UID_TEST; // KoboToolbox form ID for CB for Villagers
const KOBO_DELETE_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_1A5b_FORM_ID}/data/`; // KoboToolbox API endpoint for deleting submissions
const KOBO_DOWNLOAD_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_1A5b_FORM_ID}/data.json`; // KoboToolbox API endpoint for downloading submissions


//Download new data from Kobo Toolbox
async function downloadForm1A5bSubmissionDataFromKoboToolbox() {
    let db;
    try {
        db = getDBConnection();
        ...
        // Clear old data
        await runQuery(db, "DELETE FROM tb_Form_1A5b_Participant");
        await runQuery(db, "DELETE FROM tb_Form_1A5b_Submission");
        ...
        while (nextUrl) {
            ...
            for (const el of root.results) {
                ...
                // Insert submission
                await runQuery(db, `
                    INSERT INTO tb_Form_1A5b_Submission (
                        Id, Uuid, Start, End, Reporting_period,
                        Province, District, Village, SubActivity,
                        Conduct_date1, Conduct_date2, SchoolName, Conducted_by,
                        MaleStudent, FemaleStudent,
                        CropType, CropQuantity,
                ...
                `, [
                    ...
                    parseInt(el["group_mk3aq06/_numberofmalestudent"] || 0),
                    parseInt(el["group_mk3aq06/_numberoffemalestudent"] || 0),
                    el["group_ed3hi50/group_crops/text_ap8gb20"] || null,
                    parseInt(el["group_ed3hi50/group_crops/integer_wf4qp62"] || 0),
                    el["group_ed3hi50/group_livestock/text_wd1ml59"] || null,
                    parseInt(el["group_ed3hi50/group_livestock/integer_vv1mn28"] || 0),
                ...
                if (Array.isArray(participants)) {
                    for (const p of participants) {
                        await runQuery(db, `
                            INSERT INTO tb_Form_1A5b_Participant (
                                SubmissionId, NameAndSurname
                            ) VALUES (?, ?)
                        `, [
                            submissionId,
        ...
        console.log("✅ Form 1A5b submission data downloaded and saved.");
    } catch (err) {
        console.error("❌ Error downloading Form 1A5b data:", err.message);
        throw err;
    } finally {
...
function getForm1A5bParticipantData(language) {
    return new Promise((resolve, reject) => {
        ...
        if (language === 'LA') {
            query = `
                WITH NumberedParticipants AS (
                    ...
                    FROM tb_Form_1A5b_Participant p
                    JOIN tb_Form_1A5b_Submission s ON p.SubmissionId = s.Id
                )
                SELECT
                    np.Id AS SubmissionID,
                    np.Reporting_period AS 'ໄລຍະເວລາລາຍງານ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.Province LIMIT 1) AS 'ແຂວງ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.District LIMIT 1) AS 'ເມືອງ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.Village LIMIT 1) AS 'ບ້ານ',
					np.SubActivity AS 'ກິດຈະກຳຍ່ອຍທີ່ເຂົ້າຮ່ວມ',
                    np.Conduct_date1 AS 'ວັນເລີ່ມ',
                    np.Conduct_date2 AS 'ວັນສຳເລັດ',
					np.SchoolName AS 'ຊື່ໂຮງຮຽນ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.Conducted_by LIMIT 1) AS 'ຜູ້ດຳເນີນການ',					
					CASE WHEN np.rn = 1 THEN np.MaleStudent ELSE NULL END AS 'ນັກຮຽນຊາຍ',
					CASE WHEN np.rn = 1 THEN np.FemaleStudent ELSE NULL END AS 'ນັກຮຽນຍິງ',
					CASE WHEN np.rn = 1 THEN np.CropType ELSE NULL END AS 'ປະເພດພືດ',
					CASE WHEN np.rn = 1 THEN np.CropQuantity ELSE NULL END AS 'ຈຳນວນພືດ (ກິໂລ)',
					CASE WHEN np.rn = 1 THEN np.LivestockType ELSE NULL END AS 'ປະເພດສັດ',
					CASE WHEN np.rn = 1 THEN np.LivestockQuantity ELSE NULL END AS 'ຈຳນວນສັດ (ກິໂລ)',
            ...
            query = `
                ...
                SELECT
                    ...
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.Province LIMIT 1) AS 'Province',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.District LIMIT 1) AS 'District',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.Village LIMIT 1) AS 'Village',
					np.SubActivity AS 'Sub-Activity',
                    np.Conduct_date1 AS 'Start Date',
                    np.Conduct_date2 AS 'End Date',
					np.SchoolName AS 'School Name',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5b' AND ItemCode=np.Conducted_by LIMIT 1) AS 'Conducted By',
					CASE WHEN np.rn = 1 THEN np.MaleStudent ELSE NULL END AS 'Male Students',
					CASE WHEN np.rn = 1 THEN np.FemaleStudent ELSE NULL END AS 'Female Students',
...
function getForm1A5bParticipantDataBySID(SubmissionId, language) {
    return new Promise((resolve, reject) => {
...
// ############################ Function to delete Form 1A5b submission data from both local database and KoboToolbox Online ############################
async function deleteSubmissionInKoboAndDatabase(submissionId) {
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            db.run("DELETE FROM tb_Form_1A5b_Participant WHERE SubmissionId = ?", [submissionId], function (err) {
                if (err) return reject(err);

                // Then delete the submission itself
                db.run("DELETE FROM tb_Form_1A5b_Submission WHERE Id = ?", [submissionId], function (err) {
                    db.close();
                    if (err) return reject(err);
                    resolve();
                });
...
async function getForm1A5bSubmissionUUIDBySubmissionId(submissionId) {
    const db = getDBConnection();

    const row = await runGet(db, "SELECT Uuid FROM tb_Form_1A5b_Submission WHERE Id = ?", [submissionId]);
    db.close();
...
async function getForm1A5bNewSubmissionIdByUUID(uuId) {
    const db = getDBConnection();

    const row = await runGet(db, "SELECT Id as SubmissionId FROM tb_Form_1A5b_Submission WHERE Uuid = ?", [uuId]);
    db.close();
...
async function deleteOnlyForm1A5bParticipantInDB(participantId) {
    const db = getDBConnection();

    return new Promise((resolve, reject) => {
        db.run("DELETE FROM tb_Form_1A5b_Participant WHERE Id = ?", [participantId], function (err) {
            if (err) reject(err);
            else resolve();
        });
    });
...
async function deleteOnlyForm1A5bSubmissionInKobo(submissionId) {
    try {
        // Delete the whole submission from Kobo
...
async function getRawSubmissionAndParticipantsData(submissionId) {
    ...
    const submission = await runGet(db, "SELECT * FROM tb_Form_1A5b_Submission WHERE Id = ?", [submissionId]);
    const participants = await runAll(db, "SELECT * FROM tb_Form_1A5b_Participant WHERE SubmissionId = ?", [submissionId]);
