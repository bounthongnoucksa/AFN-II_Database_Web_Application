...
const KOBO_DOWNLOAD_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_3Act2FORM_ID}/data.json?limit=1000`; // KoboToolbox API endpoint for downloading submissions
        ...
        if (language === 'LA') {
            query = `
                ...
                SELECT
                    ...
                    CASE WHEN np.rn = 1 THEN (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='all_forms' AND ItemCode=np.CSO_Head_Ethnicity LIMIT 1) ELSE NULL END AS 'ຊົນເຜົ່າ',
                    ...
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='all_forms' AND ItemCode=np.Ethnicity LIMIT 1) AS 'ຊົນເຜົ່າ ສະມາຊິກ',
            ...
            query = `
                ...
                SELECT
                    ...
                    CASE WHEN np.rn = 1 THEN (SELECT Label_English FROM Translation_EN_LA WHERE FormName='all_forms' AND ItemCode=np.CSO_Head_Ethnicity LIMIT 1) ELSE NULL END AS 'Ethnicity',
                    ...
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='all_forms' AND ItemCode=np.Ethnicity LIMIT 1) AS 'Participant Ethnicity',
                    CASE WHEN np.rn = 1 THEN np.MAF ELSE NULL END AS 'MAF',
        ...
        if (language === 'LA') {
            ...
            query = `
                ...
                SELECT
                    ...
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='all_forms' AND ItemCode=np.Ethnicity LIMIT 1) AS 'P. Ethnicity',
                    CASE WHEN np.rn = 1 THEN np.MAF ELSE NULL END AS 'MAF',
                    CASE WHEN np.rn = 1 THEN np.CSO_contribution ELSE NULL END AS 'CSO Contribution',
                    CASE WHEN np.rn = 1 THEN np.CSO_cash ELSE NULL END AS 'CSO Cash',
                    CASE WHEN np.rn = 1 THEN np.OtherFund ELSE NULL END AS 'Other Funds'
