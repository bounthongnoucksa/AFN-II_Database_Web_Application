...
const KOBO_FORM_1A3b_FORM_ID = process.env.KOBO_FORM_1A3b_UID;
//const KOBO_FORM_1A3b_FORM_ID = process.env.KOBO_FORM_1A3b_UID_TEST; // KoboToolbox form ID for CB for Villagers
const KOBO_DELETE_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_1A3b_FORM_ID}/data/`; // KoboToolbox API endpoint for deleting submissions
const KOBO_DOWNLOAD_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_1A3b_FORM_ID}/data.json`; // KoboToolbox API endpoint for downloading submissions


//Download new data from Kobo Toolbox
async function downloadForm1A3bSubmissionDataFromKoboToolbox() {
    let db;
    try {
        db = getDBConnection();
        const headers = { Authorization: `Token ${KOBO_API_KEY}` };
        let nextUrl = KOBO_DOWNLOAD_SUBMISSION_API; // Replace with your actual API URL

        // Clear old data
        await runQuery(db, "DELETE FROM tb_Form_1A3b_Participant");
        await runQuery(db, "DELETE FROM tb_Form_1A3b_Submission");
        ...
        while (nextUrl) {
            ...
            for (const el of root.results) {
                ...
                // Insert into tb_Form_1A3b_Submission
                await runQuery(db, `
                    INSERT INTO tb_Form_1A3b_Submission 
                    (Id, Uuid, Start, End, Reporting_period, Province, District, Village,
                     Storage_built, Conduct_date1, Conduct_date2, Conducted_by, 
                     CommityExist, InitialRice, RiceBorrowed, RiceLeftInStock, IFAD, MAF, WFP, GoL, Ben, OtherFund, Version, Submission_time)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ON CONFLICT(Id) DO UPDATE SET 
                        ...
                        Storage_built=excluded.Storage_built,
                        Conduct_date1=excluded.Conduct_date1,
                        Conduct_date2=excluded.Conduct_date2,
                        Conducted_by=excluded.Conducted_by,
                        CommityExist=excluded.CommityExist,
                        InitialRice=excluded.InitialRice,
                        RiceBorrowed=excluded.RiceBorrowed,
                        RiceLeftInStock=excluded.RiceLeftInStock,
                        IFAD=excluded.IFAD,
                        MAF=excluded.MAF,
                        WFP=excluded.WFP,
                        GoL=excluded.GoL,
                ...
                `, [
                    ...
                    el["select_one_ko2sg91"] || null,
                    parseInt(el["decimal_pw8pb54"] || 0),
                    parseInt(el["decimal_jd5yh26"] || 0),
                    parseInt(el["decimal_hu8ua08"] || 0),
                ...
                if (Array.isArray(participants)) {
                    for (const p of participants) {
                        ...
                        await runQuery(db, `
                            INSERT INTO tb_Form_1A3b_Participant 
                            (SubmissionId, HaveHH_id, HHId, NameAndSurname, Age, Gender, ParticipantRole, 
                        ...
                        `, [
                            ...
                            p["group_participantdetail_hp48r4/_participantionRole"] || null,
        ...
        console.log("✅ Form 1A3b submission data downloaded and saved to the database successfully.");
    } catch (err) {
        console.error("❌ Error downloading Form 1A3b data:", err.message);
...
function getForm1A3bParticipantData(language) {
    return new Promise((resolve, reject) => {
        ...
        if (language === 'LA') {
            query = `
                WITH NumberedParticipants AS (
                    ...
                    FROM tb_Form_1A3b_Participant p
                    JOIN tb_Form_1A3b_Submission s ON p.SubmissionId = s.Id
                )
                SELECT
                    np.Id AS SubmissionID,
                    np.Reporting_period AS 'ໄລຍະເວລາລາຍງານ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Province LIMIT 1) AS 'ແຂວງ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.District LIMIT 1) AS 'ເມືອງ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Village LIMIT 1) AS 'ບ້ານ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Storage_built LIMIT 1)  AS 'ສ້າງສາງເກັບມ້ຽນເຂົ້າບໍ',
                    np.Conduct_date1 AS 'ວັນເລີ່ມ',
                    np.Conduct_date2 AS 'ວັນສຳເລັດ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Conducted_by LIMIT 1) AS 'ຮັບຜິດຊອບໂດຍ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.CommityExist LIMIT 1) AS 'ມີຄະນະຮັບຜິດຊອບບໍ',
					CASE WHEN np.rn = 1 THEN np.InitialRice ELSE NULL END AS 'ເຂົ້າຕົ້ນທຶນ',
                    CASE WHEN np.rn = 1 THEN np.RiceBorrowed ELSE NULL END AS 'ເຂົ້າທີ່ຖືກຢືມແຕ່ເລີ່ມຕົ້ນ',
                    CASE WHEN np.rn = 1 THEN np.RiceLeftInStock ELSE NULL END AS 'ເຂົ້າຍັງເຫຼືອໃນທະນາຄານ',

                    np.HHId AS 'ລະຫັດຄົວເຮືອນ',
                    np.NameAndSurname AS 'ຊື່ ແລະ ນາມສະກຸນ',
                    np.Age AS 'ອາຍຸ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Gender LIMIT 1) AS 'ເພດ',
					(SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.ParticipantRole LIMIT 1) AS 'ບົດບາດການເຂົ້າຮ່ວມ',
					(SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Ethnicity LIMIT 1) AS 'ຊົນເຜົ່າ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Poverty_level LIMIT 1) AS 'ລະດັບຄວາມທຸກຍາກ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.PWD_status LIMIT 1) AS 'ຜູ້ພິການບໍ',
            ...
            query = `
                ...
                SELECT
                    ...
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Province LIMIT 1) AS 'Province',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.District LIMIT 1) AS 'District',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Village LIMIT 1) AS 'Village',
					(SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Storage_built LIMIT 1)  AS 'Storage Constructed',
                    np.Conduct_date1 AS 'Start Date',
                    np.Conduct_date2 AS 'End Date',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Conducted_by LIMIT 1) AS 'Conducted By',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.CommityExist LIMIT 1) AS 'Rice Bank Committee',
					CASE WHEN np.rn = 1 THEN np.InitialRice ELSE NULL END AS 'Rice Initial Amnt.',
                    CASE WHEN np.rn = 1 THEN np.RiceBorrowed ELSE NULL END AS 'Cum. Amnt. borrowed rice',
                    CASE WHEN np.rn = 1 THEN np.RiceLeftInStock ELSE NULL END AS 'Present stock available',

                    np.HHId AS 'HH-ID',
                    np.NameAndSurname AS 'Participant Name',
                    np.Age AS 'Age',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Gender LIMIT 1) AS 'Gender',
					(SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.ParticipantRole LIMIT 1) AS 'Participant Role',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Ethnicity LIMIT 1) AS 'Ethnicity',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.Poverty_level LIMIT 1) AS 'Poverty Level',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a3b' AND ItemCode=np.PWD_status LIMIT 1) AS 'PWD Status',
...
function getForm1A3bParticipantDataBySID(SubmissionId, language) {
    return new Promise((resolve, reject) => {
...
// ############################ Function to delete Form 1A3b submission data from both local database and KoboToolbox Online ############################
async function deleteSubmissionInKoboAndDatabase(submissionId) {
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            db.run("DELETE FROM tb_Form_1A3b_Participant WHERE SubmissionId = ?", [submissionId], function (err) {
                if (err) return reject(err);

                // Then delete the submission itself
                db.run("DELETE FROM tb_Form_1A3b_Submission WHERE Id = ?", [submissionId], function (err) {
                    db.close();
                    if (err) return reject(err);
                    resolve();
                });
...
async function getForm1A3bSubmissionUUIDBySubmissionId(submissionId) {
    const db = getDBConnection();

    const row = await runGet(db, "SELECT Uuid FROM tb_Form_1A3b_Submission WHERE Id = ?", [submissionId]);
    db.close();
...
async function getForm1A3bNewSubmissionIdByUUID(uuId) {
    const db = getDBConnection();

    const row = await runGet(db, "SELECT Id as SubmissionId FROM tb_Form_1A3b_Submission WHERE Uuid = ?", [uuId]);
    db.close();
...
async function deleteOnlyForm1A3bParticipantInDB(participantId) {
    const db = getDBConnection();

    return new Promise((resolve, reject) => {
        db.run("DELETE FROM tb_Form_1A3b_Participant WHERE Id = ?", [participantId], function (err) {
            if (err) reject(err);
            else resolve();
        });
    });
...
async function deleteOnlyForm1A3bSubmissionInKobo(submissionId) {
    try {
        // Delete the whole submission from Kobo
...
async function getRawSubmissionAndParticipantsData(submissionId) {
    ...
    const submission = await runGet(db, "SELECT * FROM tb_Form_1A3b_Submission WHERE Id = ?", [submissionId]);
    const participants = await runAll(db, "SELECT * FROM tb_Form_1A3b_Participant WHERE SubmissionId = ?", [submissionId]);
