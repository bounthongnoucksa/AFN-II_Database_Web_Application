...
const KOBO_FORM_1A5a_FORM_ID = process.env.KOBO_FORM_1A5a_UID;
//const KOBO_FORM_1A5a_FORM_ID = process.env.KOBO_FORM_1A5a_UID_TEST; // KoboToolbox form ID for CB for Villagers
const KOBO_DELETE_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_1A5a_FORM_ID}/data/`; // KoboToolbox API endpoint for deleting submissions
const KOBO_DOWNLOAD_SUBMISSION_API = `${process.env.KOBO_API_URL}/${KOBO_FORM_1A5a_FORM_ID}/data.json`; // KoboToolbox API endpoint for downloading submissions


//Download new data from Kobo Toolbox
async function downloadForm1A5aSubmissionDataFromKoboToolbox() {
    let db;
    try {
        db = getDBConnection();
        ...
        while (nextUrl) {
            ...
            for (const el of root.results) {
                ...
                await runQuery(db, `
                    INSERT INTO tb_Form_1A5a_Submission (
                        Id, Uuid, Start, End, Reporting_period,
                        Province, District, Village, SubActivity,
                        Conduct_date1, Conduct_date2, Approach,
                        Conducted_by, IFAD, MAF, WFP, GoL, Ben,
                        OtherFund, Version, Submission_time
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                if (Array.isArray(participants)) {
                    for (const p of participants) {
                        ...
                        await runQuery(db, `
                            INSERT INTO tb_Form_1A5a_Participant (
                                SubmissionId, HaveHH_id, HHId, NameAndSurname,
                                Age, Gender, Ethnicity, Poverty_level, PWD_status
                            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                        `, [
                            submissionId,
                            haveHH,
        ...
        if (language === 'LA') {
            query = `
                ...
                SELECT
                    ...
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Province LIMIT 1) AS 'ແຂວງ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.District LIMIT 1) AS 'ເມືອງ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Village LIMIT 1) AS 'ບ້ານ',
					np.SubActivity AS 'ກິດຈະກຳຍ່ອຍທີ່ເຂົ້າຮ່ວມ',
                    np.Conduct_date1 AS 'ວັນເລີ່ມ',
                    np.Conduct_date2 AS 'ວັນສຳເລັດ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Approach LIMIT 1) AS 'ວິທີການ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Conducted_by LIMIT 1) AS 'ຮັບຜິດຊອບໂດຍ',
                    
                    np.HHId AS 'ລະຫັດຄົວເຮືອນ',
                    np.NameAndSurname AS 'ຊື່ ແລະ ນາມສະກຸນ ຜູ້ເຂົ້າຮ່ວມ',
                    np.Age AS 'ອາຍຸ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Gender LIMIT 1) AS 'ເພດ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Ethnicity LIMIT 1) AS 'ຊົນເຜົ່າ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Poverty_level LIMIT 1) AS 'ລະດັບຄວາມທຸກຍາກ',
                    (SELECT Label_Lao FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.PWD_status LIMIT 1) AS 'ຜູ້ພິການບໍ',
            ...
            query = `
                ...
                SELECT
                    ...
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Province LIMIT 1) AS 'Province',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.District LIMIT 1) AS 'District',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Village LIMIT 1) AS 'Village',
					np.SubActivity AS 'Sub-Activity',
                    np.Conduct_date1 AS 'Start Date',
                    np.Conduct_date2 AS 'End Date',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Approach LIMIT 1) AS 'Approach',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Conducted_by LIMIT 1) AS 'Conducted By',

                    np.HHId AS 'HH-ID',
                    np.NameAndSurname AS 'Participant Name',
                    np.Age AS 'Age',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Gender LIMIT 1) AS 'Gender',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Ethnicity LIMIT 1) AS 'Ethnicity',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.Poverty_level LIMIT 1) AS 'Poverty Level',
                    (SELECT Label_English FROM Translation_EN_LA WHERE FormName='form_1a5a' AND ItemCode=np.PWD_status LIMIT 1) AS 'PWD Status',
...
function getForm1A5aParticipantDataBySID(SubmissionId, language) {
    return new Promise((resolve, reject) => {
...
// ############################ Function to delete Form 1A5a submission data from both local database and KoboToolbox Online ############################
async function deleteSubmissionInKoboAndDatabase(submissionId) {
    try {
        ...
        await new Promise((resolve, reject) => {
            ...
            db.run("DELETE FROM tb_Form_1A5a_Participant WHERE SubmissionId = ?", [submissionId], function (err) {
                if (err) return reject(err);

                // Then delete the submission itself
                db.run("DELETE FROM tb_Form_1A5a_Submission WHERE Id = ?", [submissionId], function (err) {
                    db.close();
                    if (err) return reject(err);
                    resolve();
                });
...
async function getForm1A5aSubmissionUUIDBySubmissionId(submissionId) {
    const db = getDBConnection();

    const row = await runGet(db, "SELECT Uuid FROM tb_Form_1A5a_Submission WHERE Id = ?", [submissionId]);
    db.close();
...
async function getForm1A5aNewSubmissionIdByUUID(uuId) {
    const db = getDBConnection();

    const row = await runGet(db, "SELECT Id as SubmissionId FROM tb_Form_1A5a_Submission WHERE Uuid = ?", [uuId]);
    db.close();
...
async function deleteOnlyForm1A5aParticipantInDB(participantId) {
    const db = getDBConnection();

    return new Promise((resolve, reject) => {
        db.run("DELETE FROM tb_Form_1A5a_Participant WHERE Id = ?", [participantId], function (err) {
            if (err) reject(err);
            else resolve();
        });
    });
...
async function deleteOnlyForm1A5aSubmissionInKobo(submissionId) {
    try {
        // Delete the whole submission from Kobo
...
async function getRawSubmissionAndParticipantsData(submissionId) {
    ...
    const submission = await runGet(db, "SELECT * FROM tb_Form_1A5a_Submission WHERE Id = ?", [submissionId]);
    const participants = await runAll(db, "SELECT * FROM tb_Form_1A5a_Participant WHERE SubmissionId = ?", [submissionId]);
