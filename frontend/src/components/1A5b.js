// frontend/src/components/1A5b.js
...
export default function Form1A5b({ refreshTrigger }) {
    ...
    const fetchData = async (lang) => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:3001/api/form1A5b/getParticipantData?lang=${lang}`);
            if (res.data.success) {
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/form1A5b/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    //     //             const response = await axios.delete(`http://localhost:3001/api/form1A5b/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3001/api/form1A5b/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/form1A5b/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_1A5b_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 1A5b Export to Export failed');
        }
    };


    //Download data from KoboToolbox to local database
    const handleDownloadForm1A5bDataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form1A5b/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 1A5b data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 1A5b data from KoboToolbox:', error);
            //alert('Error downloading Form 1A5b data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 1A5b data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/api/form1A5b/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    `http://localhost:3001/api/form1A5b/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form1A5b/deleteParticipant', { participantId, submissionId });
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form1A5b/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">1A.5b: ກິດຈະກຳອາຫານໂຮງຮຽນ ໂດຍນຳໃຊ້ຜົນຜະລິດຂອງຄົວເຮືອນ</div>
                    <div>HGSF</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm1A5bDataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
