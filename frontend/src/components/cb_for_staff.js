// frontend/src/components/cb_for_staff.js
// This component handles the CB for Staff functionality, including data fetching, display, and interaction
// It allows staff to view, examine, and export data related to the AFN-II project capacity building database.
...
export default function CBForStaff({ refreshTrigger }) {
    ...
    const fetchData = async (lang, pageNumber = page, limit = pageSize, filters = []) => {
        ...
        try {
            ...
            const res = await axios.get(`${APP_API_URL}/api/cbForStaff/getParticipantData`, {
                params: {
                    lang,
                    page: pageNumber,
                    limit,
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/cbForStaff/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    useEffect(() => {
        ...
        if (filters && filters.length > 0) {
            ...
            fetchData(language, 1, 999999, filters);
        } else {
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/cbForStaff/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(APP_API_URL + `/api/cbForStaff/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'CB_for_Staff_Exported_Report.xlsx');
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/cbForStaff/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/cbForStaff/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">ໂຄງການກະສິກຳ ເພື່ອໂພຊະນາການ ສ້າງຄວາມເຂັ້ມແຂງຂັ້ນສູນກາງ, ຂັ້ນແຂວງ, ຂັ້ນເມືອງ</div>
                    <div >AFN2 Project Capacity Building Database for Government and other staff</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div className="d-flex align-items-center flex-wrap">
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} title='To export the data to Excel template file' onClick={handleExcelExport}>Export</button>
