// frontend/src/components/1BAct6.js
...
import { dropdownFieldsByIdx, dropdownOptions, dropdownReverseMap } from '../dropdownMap/1BAct6_dropdownFieldMapping';



export default function Form1BAct6({ refreshTrigger }) {
    const [data, setData] = useState([]);
    const [language, setLanguage] = useState('LA'); // default language
    const [loading, setLoading] = useState(false);
    const [pageSize, setpageSize] = useState(100); // limit the result to 100 records by default. if want to change this value then need to change on refresh button below as well.
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/form1BAct6/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    const handleDeleteSubmission = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete submission ID: ${submissionID}? This will remove all associated data.`)) {
            try {
                ...
                const response = await axios.delete(APP_API_URL + `/api/form1BAct6/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/form1BAct6/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1BAct6/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_1BAct6_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 1BAct6 Export to Excel failed');
        } finally {
    ...
    const handleDownloadForm1BAct6bDataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form1BAct6/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 1BAct6 data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 1BAct6 data from KoboToolbox:', error);
            //alert('Error downloading Form 1BAct6 data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 1BAct6 data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1BAct6/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/form1BAct6/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    const handleDeleteParticipant = async (participantId, submissionId) => {
        ...
        try {
            ...
            const response = await axios.post(APP_API_URL + '/api/form1BAct6/deleteParticipant', { participantId, submissionId });
            ...
            if (response.data.success) {
                ...
                await handleDownloadForm1BAct6bDataFromKobo();
    ...
    const handleEditSubmissionAndParticipants = async () => {
        ...
        try {
            ...
            //const response = await axios.post(APP_API_URL + '/api/form1BAct6/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
            const response = await axios.post(APP_API_URL + '/api/form1BAct6/updateParticipantAndSubmissionData', dataToSubmit); // Send the edited data object with new mapping
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">1B-Act 6: ສ້າງຄວາມເຂັ້ມແຂງໃຫ້ແກ່ ອົງການຈັດຕັ້ງໃນຊຸມຊົນ (ກຸ່ມຜະລິດ, ກຸ່ມນຳໃຊ້ນ້ຳ  ແລະ ອື່ນໆ)</div>
                    <div>CBOs Strengthening</div>
                </div>
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            const isEditableText = (idx === 13) || (idx === 21) || (idx === 23);
