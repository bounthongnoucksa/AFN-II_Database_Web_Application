...
export default function Form3Act1a({ refreshTrigger }) {
    ...
    const fetchData = async (lang) => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:3001/api/form3Act1a/getParticipantData?lang=${lang}`);
            if (res.data.success) {
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/form3Act1a/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    //     //             const response = await axios.delete(`http://localhost:3001/api/form3Act1a/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3001/api/form3Act1a/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/form3Act1a/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
    ...
    const handleDownloadForm3Act1aDataFromKobo = async () => {
        try {
            ...
            const response = await axios.get('http://localhost:3001/api/form3Act1a/downloadFromKoboToolbox');
            if (response.data.success) {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/api/form3Act1a/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    `http://localhost:3001/api/form3Act1a/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form3Act1a/deleteParticipant', { participantId, submissionId });
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form3Act1a/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
                                        ...
                                        <tbody>
                                            {modalData.map((row, idx) => (
                                                ...
                                                >
                                                    {Object.entries(row).map(([col, value], colIdx) => (
                                                        <td key={col}>
                                                            {(colIdx >= 20 && colIdx <= 25 && !isNaN(value))
                                                                ? Number(value).toLocaleString()
                                                                : value}
                                                        </td>
                                                    ))}
