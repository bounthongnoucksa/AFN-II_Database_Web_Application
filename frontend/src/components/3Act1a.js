// frontend/src/components/3Act1a.js
...
export default function Form3Act1a({ refreshTrigger }) {
    const [data, setData] = useState([]);
    const [language, setLanguage] = useState('LA'); // default language
    const [loading, setLoading] = useState(false);
    const [pageSize, setpageSize] = useState(150); // limit the result to 150 records by default. if want to change this value then need to change on refresh button below as well.
    ...
    const fetchData = async (lang, pageNumber = page, limit = pageSize, filters = []) => {
        setLoading(true);
        try {
            //const res = await axios.get(APP_API_URL + `/api/form3Act1a/getParticipantData?lang=${lang}&limit=${limit}`);
            // Convert filters to JSON string for query param
            const filtersParam = JSON.stringify(filters);

            const res = await axios.get(`${APP_API_URL}/api/form3Act1a/getParticipantData`, {
                params: {
                    lang,
                    page: pageNumber,
                    limit,
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/form3Act1a/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    const handleDeleteSubmission = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete submission ID: ${submissionID}? This will remove all associated data.`)) {
            try {
                ...
                const response = await axios.delete(APP_API_URL + `/api/form3Act1a/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/form3Act1a/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(APP_API_URL + `/api/form3Act1a/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_3Act1a_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 3Act1a Export to Export failed');
        }
    };


    //Download data from KoboToolbox to local database
    const handleDownloadForm3Act1aDataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get(APP_API_URL + '/api/form3Act1a/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 3Act1a data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 3Act1a data from KoboToolbox:', error);
            //alert('Error downloading Form 3Act1a data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 3Act1a data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form3Act1a/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/form3Act1a/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    const handleDeleteParticipant = async (participantId, submissionId) => {
        ...
        try {
            ...
            const response = await axios.post(APP_API_URL + '/api/form3Act1a/deleteParticipant', { participantId, submissionId });
            ...
            if (response.data.success) {
                ...
                await handleDownloadForm3Act1aDataFromKobo();
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">Act 3.1a: ການວາງແຜນພັດທະນາບ້ານ</div>
                    <div>Village Development Planning</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div className="d-flex align-items-center flex-wrap">
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm3Act1aDataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
                                                ...
                                                >
                                                    {Object.entries(row).map(([col, value], colIdx) => (
                                                        financialFields.includes(col) ? null : (
                                                            <td key={col}>
                                                                {(colIdx >= 20 && colIdx <= 25 && value != null && value != '' && !isNaN(value))
                                                                    ? Number(value).toLocaleString()
                                                                    : value}
                                                            </td>
                                                        )
