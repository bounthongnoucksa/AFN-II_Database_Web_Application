// frontend/src/components/2Act2.js
...
export default function Form2Act2({ refreshTrigger }) {
    ...
    const fetchData = async (lang) => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:3001/api/form2Act2/getParticipantData?lang=${lang}`);
            if (res.data.success) {
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/form2Act2/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    //     //             const response = await axios.delete(`http://localhost:3001/api/form2Act2/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3001/api/form2Act2/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/form2Act2/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_2Act2_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 2Act2 Export to Export failed');
        }
    };


    //Download data from KoboToolbox to local database
    const handleDownloadForm2Act2DataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form2Act2/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 2Act2 data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 2Act2 data from KoboToolbox:', error);
            //alert('Error downloading Form 2Act2 data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 2Act2 data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/api/form2Act2/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    `http://localhost:3001/api/form2Act2/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form2Act2/deleteParticipant', { participantId, submissionId });
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form2Act2/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">Act 2.2: ຈັດເວທີສົນທະນາ ແລກປ່ຽນ/ສ້າງໂອກາດ ສຳລັບທຸລະກິດທີ່ມີຫຼາຍພາກສ່ວນທີ່ສົນໃຈເຂົ້າຮ່ວມ</div>
                    <div>Business Multi Stakeholders Platform - MSP</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm2Act2DataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
                            ...
                            {data.map((row, rowIdx) => (
                                ...
                                >
                                    {Object.entries(row).map(([col, value], colIdx) => (
                                        <td key={col}>
                                            {(colIdx >= 27 && colIdx <= 33 && !isNaN(value))
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            if ((idx >= 0 && idx <= 7) || (idx >= 12 && idx <= 13) || (idx >= 15 && idx <= 22) || (idx >= 28 && idx <= 34)) {
                                                colClass = 'col-lg-1';
                                            } //else if ( idx === 13) {
                                            //colClass = 'col-lg-3';
                                            //}

                                            const needsNewLine = (idx === 27);

                                            const isDateField = (idx >= 3 && idx <= 5) || idx === 24 || idx === 26;
                                            //const isEditableText = (idx >= 11 && idx <= 7) || (idx >= 13 && idx <= 18);
                                            const isEditableText = (idx === 9) || (idx === 18) || (idx === 23);
                                            const isNumericField = (idx >= 28 && idx <= 34) || (idx === 10) || idx === 11 || idx === 15 || idx === 27;
                                        ...
                                        <tbody>
                                            {modalData.map((row, idx) => (
                                                ...
                                                >
                                                    {Object.entries(row).map(([col, value], colIdx) => (
                                                        <td key={col}>
                                                            {(colIdx >= 28 && colIdx <= 34 && !isNaN(value))
                                                                ? Number(value).toLocaleString()
                                                                : value}
                                                        </td>
                                                    ))}
