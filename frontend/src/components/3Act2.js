// frontend/src/components/3Act2.js
...
export default function Form3Act2({ refreshTrigger }) {
    ...
    const handleExcelExport = async () => {
        try {
            ...
            link.href = url;
            link.setAttribute('download', 'Form_3Act2_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 3Act2 Export to Export failed');
        }
    };


    //Download data from KoboToolbox to local database
    const handleDownloadForm3Act2DataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form3Act2/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 3Act2 data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 3Act2 data from KoboToolbox:', error);
            //alert('Error downloading Form 3Act2 data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 3Act2 data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form3Act2/deleteParticipant', { participantId, submissionId });
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form3Act2/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">Act 3.2: ການເປັນຄູ່ຮ່ວມງານກັບອົງການຈັດຕັ້ງພາກສັງຄົມ</div>
                    <div>Partnership with CSO</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm3Act2DataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            if ((idx >= 0 && idx <= 7) || idx === 10 || (idx >= 12 && idx <= 14) || idx === 20 ||(idx >= 22 && idx <= 24)) {
                                                colClass = 'col-lg-1';
                                            } else if ( idx === 19) {
                                            colClass = 'col-lg-3';
                                            }
                                            ...
                                            const isDateField = (idx >= 3 && idx <= 5) || idx === 18;
                                            //const isEditableText = (idx >= 11 && idx <= 7) || (idx >= 13 && idx <= 18);
                                            const isEditableText = (idx === 9);
                                            const isNumericField = (idx >= 25 && idx <= 28) || idx === 12 || idx === 22;
                                        ...
                                        <tbody>
                                            {modalData.map((row, idx) => (
                                                ...
                                                >
                                                    {Object.entries(row).map(([col, value], colIdx) => (
                                                        <td key={col}>
                                                            {(colIdx >= 25 && colIdx <= 28 && !isNaN(value))
                                                                ? Number(value).toLocaleString()
                                                                : value}
                                                        </td>
                                                    ))}
