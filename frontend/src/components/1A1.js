// frontend/src/components/1A1.js
...
export default function Form1A1({ refreshTrigger }) {
    const [data, setData] = useState([]);
    const [language, setLanguage] = useState('LA'); // default language
    const [loading, setLoading] = useState(false);
    const [pageSize, setpageSize] = useState(100); // limit the result to 1000 records by default. if want to change this value then need to change on refresh button below as well.
    ...
    const financialFields = ['IFAD', 'MAF', 'WFP', 'GoL', 'Ben', 'Other Fund', 'OtherFund'];
    ...
    const fetchData = async (lang, pageNumber = page, limit = pageSize, filters = []) => {
        ...
        try {
            ...
            const res = await axios.get(`${APP_API_URL}/api/form1A1/getParticipantData`, {
                params: {
                    lang,
                    page: pageNumber,
                    limit,
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/form1A1/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    const handleDeleteSubmission = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete submission ID: ${submissionID}? This will remove all associated data.`)) {
            try {
                ...
                const response = await axios.delete(APP_API_URL + `/api/form1A1/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/form1A1/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleDownloadForm1A1DataFromKobo = async () => {
        try {
            ...
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 1A1 data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 1A1 data from KoboToolbox:', error);
            //alert('Error downloading CB for staff data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 1A1 data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1A1/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/form1A1/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    const handleDeleteParticipant = async (participantId, submissionId) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                await handleDownloadForm1A1DataFromKobo();
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">1A.1: ສັງລວມຂໍ້ມູນກິດຈະກຳໃນໂຮງຮຽນໂພຊະນາການຂອງຊາວກະສິກອນ (FNSs) ແລະ ການສົ່ງເສີມບົດບາດ ຂອງແມ່ຍິງ</div>
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            if ((idx >= 0 && idx <= 7) || (idx >= 11 && idx <= 16) || (idx >= 19 && idx <= 29) || (idx === 9)) {
                                                colClass = 'col-lg-1';
                                            } //else if ( idx === 9) {
                                            //colClass = 'col-lg-3';
                                            //}

                                            const needsNewLine = idx === 30;
