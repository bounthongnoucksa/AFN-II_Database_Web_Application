// frontend/src/components/3Act1b.js
...
import { dropdownFieldsByIdx, dropdownOptions, dropdownReverseMap } from '../dropdownMap/3Act1b_dropdownFieldMapping';



export default function Form3Act1b({ refreshTrigger }) {
    const [data, setData] = useState([]);
    const [language, setLanguage] = useState('LA'); // default language
    const [loading, setLoading] = useState(false);
    const [pageSize, setpageSize] = useState(100); // limit the result to 800 records by default. if want to change this value then need to change on refresh button below as well.
    ...
    const fetchData = async (lang, pageNumber = page, limit = pageSize, filters = []) => {
        setLoading(true);
        try {
            //const res = await axios.get(APP_API_URL + `/api/form3Act1b/getParticipantData?lang=${lang}&limit=${limit}`);
            // Convert filters to JSON string for query param
            const filtersParam = JSON.stringify(filters);

            const res = await axios.get(`${APP_API_URL}/api/form3Act1b/getParticipantData`, {
                params: {
                    lang,
                    page: pageNumber,
                    limit,
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/form3Act1b/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    const handleDeleteSubmission = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete submission ID: ${submissionID}? This will remove all associated data.`)) {
            try {
                ...
                const response = await axios.delete(APP_API_URL + `/api/form3Act1b/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/form3Act1b/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form3Act1b/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_3Act1b_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 3Act1b Export to Excel failed');
        } finally {
    ...
    const handleDownloadForm3Act1bDataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form3Act1b/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 3Act1b data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 3Act1b data from KoboToolbox:', error);
            //alert('Error downloading Form 3Act1b data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 3Act1b data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form3Act1b/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/form3Act1b/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    const handleDeleteParticipant = async (participantId, submissionId) => {
        ...
        try {
            ...
            const response = await axios.post(APP_API_URL + '/api/form3Act1b/deleteParticipant', { participantId, submissionId });
            ...
            if (response.data.success) {
                ...
                await handleDownloadForm3Act1bDataFromKobo();
    ...
    const handleEditSubmissionAndParticipants = async () => {
        ...
        try {
            ...
            //const response = await axios.post(APP_API_URL + '/api/form3Act1b/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
            const response = await axios.post(APP_API_URL + '/api/form3Act1b/updateParticipantAndSubmissionData', dataToSubmit); // Send the edited data object with new mapping
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">Act 3.1b: ການປະສານງານ ແລະ ວາງແຜນຮ່ວມຫຼາຍຂະແໜງການ</div>
                    <div>DNC Supports</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div className="d-flex align-items-center flex-wrap">
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm3Act1bDataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            if ((idx >= 0 && idx <= 7) || (idx >= 12 && idx <= 15) || idx === 9) {
                                                colClass = 'col-lg-1';
                                            } //else if ( idx === 19) {
                                            //colClass = 'col-lg-3';
                                            //}
                                            ...
                                            const isEditableText = (idx === 8) || (idx === 10);
                                            const isNumericField = (idx >= 18 && idx <= 23) || idx === 9 || idx === 12 || idx === 14;
