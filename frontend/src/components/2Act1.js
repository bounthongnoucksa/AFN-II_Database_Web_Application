// frontend/src/components/2Act1.js
...
export default function Form2Act1({ refreshTrigger }) {
    ...
    const fetchData = async (lang) => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:3001/api/form2Act1/getParticipantData?lang=${lang}`);
            if (res.data.success) {
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/form2Act1/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    //     //             const response = await axios.delete(`http://localhost:3001/api/form2Act1/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3001/api/form2Act1/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/form2Act1/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_2Act1_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 2Act1 Export to Export failed');
        }
    };


    //Download data from KoboToolbox to local database
    const handleDownloadForm2Act1DataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form2Act1/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 1BAct8 data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 1BAct8 data from KoboToolbox:', error);
            //alert('Error downloading Form 1BAct8 data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 1BAct8 data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/api/form2Act1/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    `http://localhost:3001/api/form2Act1/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form2Act1/deleteParticipant', { participantId, submissionId });
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form2Act1/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">Act 2.1: ສະໜັບສະໜູນ ຈຸລະວິສາຫະກິດໆ ຂະໜາດນ້ອຍ-ກາງ ໃນລະບົບຕ່ອງໂສ້ການສະໜອງສະບຽງອາຫານ</div>
                    <div>Support to MSME in food supply chains</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm2Act1DataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
                            ...
                            {data.map((row, rowIdx) => (
                                ...
                                >
                                    {Object.entries(row).map(([col, value], colIdx) => (
                                        <td key={col}>
                                            {(colIdx >= 25 && colIdx <= 31 && !isNaN(value))
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            if ((idx >= 0 && idx <= 7) || (idx >= 11 && idx <= 20) || (idx >= 23 && idx <= 24) || idx === 10) {
                                                colClass = 'col-lg-1';
                                            } //else if ( idx === 13) {
                                            //colClass = 'col-lg-3';
                                            //}
                                            ...
                                            const isDateField = (idx >= 3 && idx <= 5) || idx === 10 || idx === 20;
                                            //const isEditableText = (idx >= 11 && idx <= 7) || (idx >= 13 && idx <= 18);
                                            const isEditableText = (idx === 9) || (idx === 16) || (idx === 21) || (idx === 22) || (idx === 25);
                                            const isNumericField = (idx >= 26 && idx <= 32) || (idx === 12) || idx === 23;
                                        ...
                                        <tbody>
                                            {modalData.map((row, idx) => (
                                                ...
                                                >
                                                    {Object.entries(row).map(([col, value], colIdx) => (
                                                        <td key={col}>
                                                            {(colIdx >= 26 && colIdx <= 32 && !isNaN(value))
                                                                ? Number(value).toLocaleString()
                                                                : value}
                                                        </td>
                                                    ))}
