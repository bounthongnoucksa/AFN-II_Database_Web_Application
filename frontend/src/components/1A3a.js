// frontend/src/components/1A3a.js
...
export default function Form1A3a({ refreshTrigger }) {
    ...
    const fetchData = async (lang) => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:3001/api/form1A3a/getParticipantData?lang=${lang}`);
            if (res.data.success) {
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/form1A3a/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    //     //             const response = await axios.delete(`http://localhost:3001/api/cbForStaff/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3001/api/form1A3a/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/form1A3a/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_1A3a_Exported_Report.xlsx');
    ...
    const handleDownloadForm1A3aDataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form1A3a/downloadFromKoboToolbox');
            if (response.data.success) {
                ...
                setModalMessage('❌ Failed to download Form 1A3a data from KoboToolbox');
                setShowSuccessModalMessage(true); // Show the modal loading completed with the message
            }
        } catch (error) {
            console.error('Error downloading Form 1A3a data from KoboToolbox:', error);
            //alert('Error downloading Form 1A3a data from KoboToolbox');
            setModalMessage('❌ Error downloading Form 1A3a data from KoboToolbox');
            setShowSuccessModalMessage(true); // Show the modal loading completed with the message
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/api/form1A3a/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    `http://localhost:3001/api/form1A3a/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">1A.3a: ການປຸງແຕ່ງອາຫານຢູ່ພາຍໃນຄົວເຮືອນ/ຊຸມຊົນ (Domestic Food Processing)</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm1A3aDataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
