...
import { dropdownFieldsByIdx, dropdownOptions, dropdownReverseMap } from '../dropdownMap/1BAct7_dropdownFieldMapping';
...
export default function Form1BAct7({ refreshTrigger }) {
    ...
    const fetchData = async (lang, pageNumber = page, limit = pageSize, filters = []) => {
        setLoading(true);
        try {
            //const res = await axios.get(APP_API_URL + `/api/form1BAct7/getParticipantData?lang=${lang}&limit=${limit}`);
            // Convert filters to JSON string for query param
            const filtersParam = JSON.stringify(filters);

            const res = await axios.get(`${APP_API_URL}/api/form1BAct7/getParticipantData`, {
                params: {
                    lang,
                    page: pageNumber,
                    limit,
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/form1BAct7/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    const handleDeleteSubmission = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete submission ID: ${submissionID}? This will remove all associated data.`)) {
            try {
                ...
                const response = await axios.delete(APP_API_URL + `/api/form1BAct7/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/form1BAct7/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1BAct7/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            alert('Form 1BAct7 Export to Excel failed');
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1BAct7/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/form1BAct7/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    const handleDeleteParticipant = async (participantId, submissionId) => {
        ...
        try {
            ...
            const response = await axios.post(APP_API_URL + '/api/form1BAct7/deleteParticipant', { participantId, submissionId });
            ...
            if (response.data.success) {
                ...
                await handleDownloadForm1BAct7bDataFromKobo();
    ...
    const handleEditSubmissionAndParticipants = async () => {
        ...
        try {
            ...
            //const response = await axios.post(APP_API_URL + '/api/form1BAct7/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
            const response = await axios.post(APP_API_URL + '/api/form1BAct7/updateParticipantAndSubmissionData', dataToSubmit); // Send the edited data object with new mapping
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            const isEditableText = (idx === 11) || (idx === 40);
