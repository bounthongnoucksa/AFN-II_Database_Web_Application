...
import { dropdownFieldsByIdx, dropdownOptions, dropdownReverseMap } from '../dropdownMap/1A4_dropdownFieldMapping';
...
export default function Form1A4({ refreshTrigger }) {
    ...
    const fetchData = async (lang, pageNumber = page, limit = pageSize, filters = []) => {
        setLoading(true);
        try {
            //const res = await axios.get(APP_API_URL + `/api/form1A4/getParticipantData?lang=${lang}&limit=${limit}`);
            // Convert filters to JSON string for query param
            const filtersParam = JSON.stringify(filters);

            const res = await axios.get(`${APP_API_URL}/api/form1A4/getParticipantData`, {
                params: {
                    lang,
                    page: pageNumber,
                    limit,
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(APP_API_URL + `/api/form1A4/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    const handleDeleteSubmission = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete submission ID: ${submissionID}? This will remove all associated data.`)) {
            try {
                ...
                const response = await axios.delete(APP_API_URL + `/api/form1A4/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(APP_API_URL + `/api/form1A4/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1A4/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            alert('Form 1A4 Export to Excel failed');
        } finally {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(APP_API_URL + `/api/form1A4/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    APP_API_URL + `/api/form1A4/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    const handleDeleteParticipant = async (participantId, submissionId) => {
        ...
        try {
            ...
            const response = await axios.post(APP_API_URL + '/api/form1A4/deleteParticipant', { participantId, submissionId });
            ...
            if (response.data.success) {
                ...
                await handleDownloadForm1A4DataFromKobo();
    ...
    const handleEditSubmissionAndParticipants = async () => {
        ...
        try {
            ...
            //const response = await axios.post(APP_API_URL + '/api/form1A4/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
            const response = await axios.post(APP_API_URL + '/api/form1A4/updateParticipantAndSubmissionData', dataToSubmit); // Send the edited data object with new mapping
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div className="d-flex align-items-center flex-wrap">
                    ...
                    {/* <button className='btn btn-primary btn-sm me-2 ' style={{ width: '120px' }} onClick={() => { setpageSize(1000); }} title='To reload data from application database'>Refresh</button> */}
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            const isEditableText = (idx === 14) || (idx === 16);
