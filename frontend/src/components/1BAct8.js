// frontend/src/components/1BAct8.js
...
export default function Form1BAct8({ refreshTrigger }) {
    ...
    const fetchData = async (lang) => {
        setLoading(true);
        try {
            const res = await axios.get(`http://localhost:3001/api/form1BAct8/getParticipantData?lang=${lang}`);
            if (res.data.success) {
    ...
    const fetchModalData = async () => {
        try {
            const res = await axios.get(`http://localhost:3001/api/form1BAct8/getParticipantDataBySID?submissionId=${submissionID}&lang=${language}`);
            if (res.data.success) {
    ...
    //     //             const response = await axios.delete(`http://localhost:3001/api/form1BAct8/deleteSubmission?submissionId=${submissionID}`);
    ...
    const handleSubmissionUUID = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:3001/api/form1BAct8/getUUID?submissionId=${submissionID}`);
            if (response.data.success) {
    ...
    const handleExcelExport = async () => {
        try {
            const response = await axios.get(`http://localhost:3001/api/form1BAct8/exportToExcel?lang=${language}`, {
                responseType: 'blob',
            });
            ...
            link.href = url;
            link.setAttribute('download', 'Form_1BAct8_Exported_Report.xlsx');
            document.body.appendChild(link);
            link.click();
            link.remove();
        } catch (error) {
            console.error('Export failed', error);
            alert('Form 1BAct8 Export to Export failed');
        }
    };


    //Download data from KoboToolbox to local database
    const handleDownloadForm1BAct8bDataFromKobo = async () => {
        try {
            setLoadingModalMessage(true); //for modal loading pop state
            const response = await axios.get('http://localhost:3001/api/form1BAct8/downloadFromKoboToolbox');
            if (response.data.success) {
    ...
    const refreshModalWithNewSubmission = async (uuid, language) => {
        ...
        try {
            ...
            const response = await axios.get(`http://localhost:3001/api/form1BAct8/getNewSubmissionID?Uuid=${uuid}`);
            ...
            if (response.data.success) {
                ...
                const res = await axios.get(
                    `http://localhost:3001/api/form1BAct8/getParticipantDataBySID?submissionId=${newSubmissionId}&lang=${language}`
                );
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form1BAct8/deleteParticipant', { participantId, submissionId });
    ...
    //     //         const response = await axios.post('http://localhost:3001/api/form1BAct8/updateParticipantAndSubmissionData', selectedRow); // Send the edited data object
        ...
        <div onClick={() => setShowContextMenu(false)}>
            ...
            <div className="d-flex align-items-center justify-content-between mb-2">
                <div className="text-center w-100">
                    <div className="fw-bold fs-5">1B-Act 8: ປັບປຸງພື້ນຖານໂຄງລ່າງກະສິກຳ ເພື່ອການປັບຕົວຕໍ່ການປ່ຽນແປງດິນຟ້າອາກາດ</div>
                    <div>Climate Change Adaptation infrastructures (irrigation /MUS) built and upgraded</div>
                </div>
            ...
            <div className="d-flex justify-content-between mb-2">
                <div>
                    ...
                    <button className='btn btn-primary btn-sm me-2' style={{ width: '120px' }} onClick={handleDownloadForm1BAct8bDataFromKobo} title='To cleanup application database and reload new data from KoboToolbox online database'>Load new data</button>
                            ...
                            {data.map((row, rowIdx) => (
                                ...
                                >
                                    {Object.entries(row).map(([col, value], colIdx) => (
                                        <td key={col}>
                                            {(colIdx >= 20 && colIdx <= 27 && !isNaN(value))
                                ...
                                {selectedRow && Object.keys(selectedRow).length > 0 && (
                                    <div className="row">
                                        {Object.entries(selectedRow).map(([key, value], idx) => {
                                            ...
                                            if ((idx >= 0 && idx <= 7) || (idx >= 11 && idx <= 17) || (idx === 19) || (idx >= 23 && idx <= 28)) {
                                                colClass = 'col-lg-1';
                                            } //else if ( idx === 13) {
                                            //colClass = 'col-lg-3';
                                            //}
                                            ...
                                            const isEditableText = (idx === 30);
                                            const isNumericField = (idx >= 21 && idx <= 28) || (idx === 13) || (idx === 18);
